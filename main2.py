...
def draw(window, background, player, enemy, objects, bullet_group,
         enemy_bullet_group):
    ...
    hp_bars(enemy, player, WIDTH - 200, 50, enemy_bullet_group, bullet_group,"lightsaber")
    ...
    for bullet in enemy_bullet_group:
        if pygame.sprite.collide_mask(player, bullet) and player.block_count == 1:
            ...
            if bullet.direction == "right":
                bullet_group.add(Bullet(player.rect.right, player.rect.centery -20 , "right"))
                
                

def handle_move(player, enemy, objects):
...
def main(window):
    ...
    player = Player(100, 100, 112, 217,300, "ben_kenobi", "lightsaber")
    enemy = Player(1100, 100, 203, 223, 250, "darthvader", "lightsaber")
    # enemy2 = Player(900, 100, 133, 168, 1, "jawaa", "shot")
    ...
    while run:
        ...
        if player.rect.x != enemy.rect.x and player.rect.x < enemy.rect.x - 50:
            enemy.move_left(3)
        elif player.rect.x != enemy.rect.x and player.rect.x > enemy.rect.x + 50:
            enemy.move_right(3)
        ...
        if number % 30 == 0 and enemy.actual_health > 0 and -50< player.rect.x - enemy.rect.x < 50:
            enemy.attack_count = enemy.attack_count*-1 + 1
        ...
        enemy.loop(FPS)
        # enemy2.loop(FPS)
        handle_move(player,enemy, blocks)
        draw(window, background, player, enemy, blocks, bullet_group,enemy_bullet_group)
